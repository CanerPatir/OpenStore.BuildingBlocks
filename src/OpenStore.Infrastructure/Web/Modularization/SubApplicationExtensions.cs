...
public static class SubApplicationExtensions
{
        ...
        where TStartup : ModuleStartup
    {
        ...
        return app.Isolate(
            builder => builder.Map(path,
                subApp =>
                {
                    subAppStartup.Start(subApp, app.ApplicationServices.GetService<IHostApplicationLifetime>().ApplicationStopping).ConfigureAwait(false).GetAwaiter()
                        .GetResult();
                }), services =>
            {
